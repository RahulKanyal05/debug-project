...
// *** NEW: Variable to hold interim transcript separately for display ***
...
if (!window.SpeechRecognition) {
    ...
    // ***********************************************************************
    // *         MODIFIED SPEECH RECOGNITION 'result' EVENT LISTENER         *
    // ***********************************************************************
    recognition.addEventListener('result', (e) => {
        if (isSpeaking) return; // Ignore input if AI is talking

        let interimTranscript = '';
        let finalTranscriptPiece = ''; // Store only the *new* final part from this event

        for (let i = e.resultIndex; i < e.results.length; ++i) {
            const transcript = e.results[i][0].transcript;
            if (e.results[i].isFinal) {
                finalTranscriptPiece += transcript.trim() + ' '; // Append new final part
            } else {
                interimTranscript += transcript; // Current interim part
            }
        }

        // Store the latest interim transcript for display updates
        currentInterimTranscript = interimTranscript.trim();

        // If a new final piece was recognized, append it to the stored answer
        if (finalTranscriptPiece && currentQuestionIndex >= 0 && currentQuestionIndex < questions.length) {
            finalTranscriptPiece = finalTranscriptPiece.trim(); // Trim final piece before appending
             // Initialize if undefined/null (first final piece for this question)
            if (answers[currentQuestionIndex] === undefined || answers[currentQuestionIndex] === null) {
                 answers[currentQuestionIndex] = '';
            }
            answers[currentQuestionIndex] += finalTranscriptPiece + ' '; // *** APPEND the final piece ***
            console.log("Appended Final Transcript Piece:", finalTranscriptPiece);
            console.log("Current Full Stored Answer:", answers[currentQuestionIndex]);
            // Reset interim transcript since a final piece was just added
            currentInterimTranscript = '';
        }

        // Update the display with the full stored answer + current interim part
        updateAnswerDisplay();

    });
    // ***********************************************************************
    // *       END OF MODIFIED SPEECH RECOGNITION 'result' EVENT LISTENER    *
    // ***********************************************************************


    recognition.addEventListener('end', () => {
        console.log("Recognition ended.");
        // Continuous mode might sometimes stop unexpectedly. Restart if needed.
        // Ensure restart only happens if intended (active, not speaking, not analyzing, interview ongoing)
        if (recognitionActive && !isSpeaking && !isAnalyzingAnswers && currentQuestionIndex < questions.length) {
            console.log("Attempting to restart recognition after 'end' event (continuous mode).");
            try {
                // Prevent immediate restart if stopInterview/answerComplete was just called
                setTimeout(() => {
                    // Double-check conditions before restarting
                    if (recognitionActive && !isSpeaking && !isAnalyzingAnswers && currentQuestionIndex < questions.length) {
                        recognition.start();
                        console.log("Recognition restarted successfully after 'end'.");
                    } else {
    ...
    recognition.addEventListener('error', (event) => {
        ...
        if (event.error === 'audio-capture') {
            ...
            if (recognitionActive) stopInterview(true); // Stop if critical error
        } else if (event.error === 'not-allowed') {
            alert('Microphone access denied. Please allow microphone access in your browser settings.');
             if (recognitionActive) stopInterview(true); // Stop if critical error
        } else if (event.error === 'network') {
             alert("Network error during speech recognition. Please check your connection. Recording might be incomplete.");
             // Don't necessarily stop the interview, but inform user
        } else if (event.error === 'no-speech' && recognitionActive) {
             console.log("No speech detected, recognition might stop/restart."); // Normal in continuous mode pauses
        } else if (event.error === 'aborted') {
             console.log("Recognition aborted (likely intentional stop/completion)."); // Normal
            ...
            alert(`An unknown speech recognition error occurred: ${event.error}. Recording might stop.`);
            // Consider stopping depending on the error type if needed
         ...
         if(['audio-capture', 'not-allowed', 'service-not-allowed'].includes(event.error)) {
             ...
             // Attempt to clean up state if needed
             if (typeof recognition !== 'undefined') { try { recognition.stop(); } catch(e){} }
         }
    });

    // --- Speech Synthesis Setup ---
    // (No changes needed in speak() function for this specific requirement)
    ...
    function speak(text, callback) {
        ...
        if (recognitionActive) {
            ...
            recognitionActive = false;
            currentInterimTranscript = ''; // Clear interim display when AI starts talking
            updateAnswerDisplay(); // Update display to clear interim text
            try {
    ...
    function displayQuestion(question) {
        ...
        // Reset interim transcript when displaying a new question
        currentInterimTranscript = '';
    }

    // ***********************************************************************
    // *             MODIFIED updateAnswerDisplay Function                   *
    // ***********************************************************************
    function updateAnswerDisplay() {
        const answerElem = texts.querySelector('.answer');
        if (answerElem && !isAnalyzingAnswers && currentQuestionIndex >= 0 && currentQuestionIndex < questions.length) {
            // Construct the display string from the stored answer + current interim text
            const storedAnswer = (answers[currentQuestionIndex] || '').trim(); // Get stored answer, trim spaces
            const interimText = (currentInterimTranscript || '').trim(); // Get current interim text

            // Combine them, ensuring a space if both exist
            let displayText = storedAnswer;
            if (storedAnswer && interimText) {
                displayText += ' ' + interimText;
            } else if (interimText) {
                displayText = interimText;
            }

            // Show placeholder if nothing has been said yet
            if (!displayText) {
                 answerElem.innerText = `A: ... (Listening)`;
            } else {
                 answerElem.innerText = `A: ${displayText}`;
            }

        } else if (answerElem) {
            // Handle cases where we might not be in an active question (e.g., initial state or after stop)
             // Check if there's a final answer stored even if analyzing started (for display after stop)
            const storedAnswer = answers[currentQuestionIndex] || '';
            if(storedAnswer) {
                answerElem.innerText = `A: ${storedAnswer.trim()}`;
            } else {
                answerElem.innerText = `A: ...`;
            }
        }
    }
    // ***********************************************************************
    // *           END OF MODIFIED updateAnswerDisplay Function              *
    // ***********************************************************************
    ...
    // (No changes needed in handleAnalyzeAndPrepare for this specific requirement)
    async function handleAnalyzeAndPrepare() {
        ...
        try {
            ...
            answers = new Array(questions.length).fill(''); // Initialize with empty strings is better
    ...
    // (No changes needed in handleProceedToInterview for this specific requirement)
    ...
    // (No changes needed in these functions for this specific requirement)
    ...
    // (No changes needed in fetchQuestionsFromGemini for this specific requirement)
    async function fetchQuestionsFromGemini(resumeText, jobRole) {
     ...
     // Fallback Questions
     // (No changes needed in getFallbackQuestions for this specific requirement)
    ...
    // Greeting function
    // (No changes needed in playGreeting for this specific requirement)
    ...
    // (Minor change: Call recognition.start() here)
    function startInterviewFlow() {
        ...
        // Ask the first question (which will then start recognition *after* speaking)
        askNextQuestion();
    }
    ...
    function askNextQuestion() {
        if (currentQuestionIndex < questions.length) {
            ...
            // *** Reset answer and interim transcript for the new question ***
            answers[currentQuestionIndex] = ''; // Ensure answer slot is fresh
            currentInterimTranscript = '';    // Reset interim text

            displayQuestion(question); // Display Q, placeholder A

            recognitionActive = true; // Ensure recognition is expected to be active *after* the AI speaks

            // Speak the question. `speak` function handles stopping/restarting recognition in its `onend` callback.
            speak(question, () => {
                console.log(`Ready for answer to question ${currentQuestionIndex + 1}`);
                // Recognition restart attempt is handled by speak()'s onend/onerror + delay logic
                // Ensure it IS active and started if speak() didn't handle it (e.g., synthesis off)
                if (recognitionActive && !isSpeaking && !isAnalyzingAnswers) {
                     try {
                         // Check if recognition is *already* running (due to speak's restart logic)
                         // This check is tricky without a state variable from the API itself.
                         // We'll rely on the start() call potentially throwing 'InvalidStateError' if already running.
                         recognition.start();
                         console.log("Recognition explicitly started in askNextQuestion (if not already running).")
                     } catch (error) {
                          if (error.name !== 'InvalidStateError') {
                             console.error("Error explicitly starting recognition in askNextQuestion:", error);
                         } else {
                             console.log("Recognition likely already started by speak() callback.");
                         }
                     }
                }
            });
    ...
    // stopInterview function: Ensure final display update and recognition stop
    async function stopInterview(manuallyStopped = false) {
        ...
        recognitionActive = false; // Set flag immediately
        isAnalyzingAnswers = true;
        isSpeaking = false;
        currentInterimTranscript = ''; // Clear any lingering interim text

        if (typeof recognition !== 'undefined') {
            try {
                recognition.stop(); // Use stop() first for graceful end if possible
                console.log("Speech recognition stopped.");
            } catch (e) {
                console.warn("Error during recognition.stop(), trying abort():", e);
        ...
        // Update display one last time to show the final recorded answer without interim text
        updateAnswerDisplay();

        // --- Rest of the UI updates and analysis logic ---
        if (container) container.style.display = 'none';
        ...
        // Trim final answers before analysis
        const finalAnswersForAnalysis = answers.map(ans => (ans || '').trim());
        console.log("Final Answers Recorded for Analysis:", finalAnswersForAnalysis.map((ans, i) => `Q${i+1}: ${ans || '(empty)'}`).join('\n'));

        try {
            // Ensure questions and answers arrays are valid before analysis
            const validQuestions = Array.isArray(questions) ? questions : [];
            const validAnswers = Array.isArray(finalAnswersForAnalysis) ? finalAnswersForAnalysis : [];
    ...
    // Answer Complete button: Stop recognition, finalize display, move to next
    if (answerCompleteBtn) answerCompleteBtn.addEventListener('click', () => {
        ...
        recognitionActive = false; // Set flag
        currentInterimTranscript = ''; // Clear interim text
        if (typeof recognition !== 'undefined') {
             try {
                recognition.stop(); // Graceful stop preferred
                console.log("Recognition stopped by Answer Complete.");
             } catch(e){
                 console.warn("Error stopping recognition on Answer Complete, trying abort:", e);
        ...
        // Update display immediately to show final stored answer
        updateAnswerDisplay();

        // Short delay might not be needed anymore, but keep for safety?
        // Let's remove the delay for snappier feel, rely on `stop()` completing
        // setTimeout(() => { // REMOVED DELAY
            const finalAnswer = (answers[currentQuestionIndex] || '').trim();
            console.log(`Processing final answer for Q${currentQuestionIndex + 1}: "${finalAnswer.substring(0, 50)}..."`);
            answers[currentQuestionIndex] = finalAnswer; // Store the trimmed version
        ...
        // }, 50); // REMOVED DELAY (was 300ms)
    });

    // Submit Feedback button
    // (No changes needed for this specific requirement)
    ...
    // Download PDF button
    // (No changes needed for this specific requirement, but ensure it uses trimmed answers)
    if (downloadPdfBtn) downloadPdfBtn.addEventListener('click', () => {
        console.log("Generate PDF button clicked.");
        generatePDF(); // generatePDF will use the potentially trimmed `answers` array
    });


    // --- Answer Analysis Functions ---
    // (No changes needed in analyzeAllAnswers or getAnswerFeedbackFromGemini for this specific requirement)
    async function analyzeAllAnswers(qs, ans) {
        if (GEMINI_API_KEY === 'YOUR_GEMINI_API_KEY' || !GEMINI_API_KEY) {
         ...
         if (qs.length !== ans.length) {
             console.warn(`Mismatch between questions (${qs.length}) and answers (${ans.length}) count. Adjusting.`);
        ...
        const feedbackPromises = qs.map((question, index) => {
            const answer = ans[index] || ""; // Use already trimmed answer
    ...
    async function getAnswerFeedbackFromGemini(question, answer, questionNum, jobRole) {
        ...
        **Example Answer Approach:** [Your example answer structure/points here]
        ...
        try {
            ...
            const generatedText = candidate?.content?.parts?.[0]?.text;
            if (generatedText) {
                // --- Enhanced Parsing Logic --- (Remains the same)
                ...
                if (!grammarNotes.includes("parse")) {
    ...
    // (No changes needed in generatePDF for this specific requirement, uses final 'answers' array)
        ...
        try {
            ...
            // Helper function to add text and handle pagination
             const addText = (text, x, y, options = {}, size = 10, style = 'normal') => {
                 ...
                 // Adjust line spacing factor slightly for different styles maybe?
                 const textHeight = lines.length * lineHeight * (style === 'bold' || style === 'bolditalic' ? lineSpacingFactor * 0.95 : lineSpacingFactor);
            ...
            if (!questions || questions.length === 0 || !Array.isArray(questions)) {
                ...
                // Use the final, potentially trimmed answers from the state
                const finalAnswers = answers.map(a => (a || '').trim());

                questions.forEach((question, index) => {
                    const qText = `Q${index + 1}: ${question}`;
                    const aText = finalAnswers[index] || '(No answer recorded)'; // Use trimmed answer
                    ...
                    // Estimate block height (adjust if needed based on typical feedback length)
                    const estimatedBlockHeight = 20 // Question
                                               + (Math.ceil(aText.length / 70) * 5) // Answer lines
                                               + 50; // Feedback section (approx)
                    if (yPos + estimatedBlockHeight > pageHeight - margin && index > 0) { // Don't add page before first Q
                        doc.addPage();
                        yPos = margin;
                    ...
                    const answerX = margin + 3;
                    const answerMaxWidth = contentWidth - 3;
                    doc.setFontSize(9);
                    doc.setFont(undefined, 'normal');
                    const answerLines = doc.splitTextToSize(`Answer: ${aText}`, answerMaxWidth);
                    const answerLineHeight = doc.getLineHeight(`Answer: ${aText}`) / doc.internal.scaleFactor;
                    const answerTextHeight = answerLines.length * answerLineHeight * lineSpacingFactor;
                     if (yPos + answerTextHeight > pageHeight - margin) {
                        doc.addPage(); yPos = margin;
                        yPos = addText("... Continued Answer:", answerX, yPos, {}, 9, 'italic'); // Indicate continuation
                    }
                    doc.text(answerLines, answerX, yPos);
                    yPos += answerTextHeight;
                    yPos += feedbackItemSpacing + 1; // More space before feedback section


                    // --- Print Feedback Section ---
                    const feedbackX = margin + feedbackIndent;
                    const feedbackMaxWidth = contentWidth - feedbackIndent - 2; // Max width for feedback content

                    if (feedback) {
                         if (feedback.error) {
                             yPos = addText(`Feedback Status: ${feedback.error}`, feedbackX, yPos, { textColor: '#D32F2F', maxWidth: feedbackMaxWidth }, 9, 'italic');
                         } else {
                             // Display Analysis
                             yPos = addText(`Analysis:`, feedbackX, yPos, {}, 9, 'bolditalic');
                             yPos = addText(feedback.analysis || 'N/A', feedbackX + 2, yPos, { maxWidth: feedbackMaxWidth }, 9, 'italic');
                             yPos += feedbackItemSpacing;

                             // Display Suggestion
                             yPos = addText(`Suggestion:`, feedbackX, yPos, {}, 9, 'bolditalic');
                             yPos = addText(feedback.suggestion || 'N/A', feedbackX + 2, yPos, { maxWidth: feedbackMaxWidth }, 9, 'italic');
                             yPos += feedbackItemSpacing;

                             // --- Display Grammar Notes ---
                             yPos = addText(`Grammar & Phrasing:`, feedbackX, yPos, {}, 9, 'bolditalic');
                             yPos = addText(feedback.grammarNotes || 'N/A', feedbackX + 2, yPos, { maxWidth: feedbackMaxWidth }, 9, 'italic');
                             yPos += feedbackItemSpacing;

                             // --- Display Example Answer Approach ---
                             yPos = addText(`Example Answer Approach:`, feedbackX, yPos, {}, 9, 'bolditalic');
                             yPos = addText(feedback.exampleAnswer || 'N/A', feedbackX + 2, yPos, { maxWidth: feedbackMaxWidth }, 9, 'italic');
                             // No extra spacing needed after the last item
                         }
                     } else {
                        yPos = addText(`(Feedback data not available)`, feedbackX, yPos, { textColor: '#888888', maxWidth: feedbackMaxWidth }, 9, 'italic');
                     }
            ...
            if (currentUserFeedback && currentUserFeedback.trim()) {
                ...
                const estimatedFeedbackHeight = 10 + (Math.ceil(currentUserFeedback.length / 80) * 5); // Estimate height
    ...
    // resetInterviewState: Ensure interim transcript is cleared
    function resetInterviewState() {
        ...
        recognitionActive = false;
        isSpeaking = false;
        currentInterimTranscript = ''; // *** Clear interim transcript ***
        // Don't reset currentJobRole
    }

    // resetToUploadState: Ensure UI is fully reset
    function resetToUploadState() {
         console.log("Resetting UI to upload state.");
         resetInterviewState(); // Clear state, including interim transcript

         // Stop processes
          if (typeof recognition !== 'undefined') {
              recognitionActive = false; // Ensure flag is false before stopping
              try { recognition.abort(); } catch(e){} // Abort is safer for immediate stop
          }
         ...
         if(uploadSection) uploadSection.style.display = 'block';
         hideLoading();
         if(texts) texts.innerHTML = ''; // Clear Q&A display area
         ...
         if(jobRoleSelect) jobRoleSelect.selectedIndex = 0; // Reset dropdown
